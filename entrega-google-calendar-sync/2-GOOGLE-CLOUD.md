...
3. Nombre del proyecto: por ejemplo `CRM Calendar Sync`. Organización: déjala vacía si
...
Ejemplos, sustituye por tu dominio real:

- Producción (ejemplo): `https://crm.tu-dominio.com/?entryPoint=gcsCallback`
- Entorno local de pruebas (ejemplo): `http://localhost:8080/?entryPoint=gcsCallback`
...
`logger.level` a `DEBUG` en `data/config.php` si necesitas más detalle.
